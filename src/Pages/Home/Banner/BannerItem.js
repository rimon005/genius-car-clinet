import React from 'react';

const BannerItem = ({slide}) => {
    const {image , id , next , prev} = slide
    return (
        <div id={`slide${id}`} className="carousel-item relative w-full">
            <div className="carousel-img">
                <img src={image} alt='' className="w-full rounded-xl" />
            </div>
            <div className="absolute flex justify-end transform -translate-y-1/2 left-24 top-1/4">
                <h1 className='text-white text-6xl font-bold banner-heading'>
                    Affordable <br />
                    Price For Car <br />
                    Servicing
                </h1>
            </div>
            <div className="w-1/2 absolute flex justify-end transform -translate-y-1/2 left-24 top-1/2">
                <p className='text-xl text-white'>There are many variations of passages of  available, but the majority have suffered alteration in some form</p>
            </div>
            <div className="w-1/2 absolute flex justify-start transform -translate-y-1/2 left-24 top-3/4">
                <button className="btn discover-btn rounded-none mr-5">Discover More</button>
                <button className="btn btn-outline project-btn rounded-none">Latest Project</button>
            </div>
            <div className="absolute flex justify-end transform -translate-y-1/2 left-5 right-5 bottom-0">
                <a href={`#slide${prev}`} className="btn btn-circle mr-5 bg-orange-500 text-white hover:bg-orange-600 border-none">❮</a>
                <a href={`#slide${next}`} className="btn btn-circle bg-orange-600 text-white hover:bg-orange-600 border-none">❯</a>
            </div>
        </div>
    );
};

export default BannerItem;